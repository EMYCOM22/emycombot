...
const ADMIN_NUMBERS = [
    '551137777212@c.us',  // 👈 SEU NÚMERO CORRIGIDO (sem o +)
];
...
function isAdmin(numero) {
    // Verifica tanto com @c.us quanto sem
...
const ETAPA_DO_CLIENTE = new Map();
...
const FLUXOS = {
    ...
    teste: {
        boasVindas: [
            ...
            "Mais de 15.000 canais disponíveis!"
        ],
        perguntas: [
    ...
    apps: {
        boasVindas: [
            ...
            "Os principais são: XCIPTV, TiviMate, IPTV Smarters e OTT Navigator."
        ],
        perguntas: [
    ...
    pagamento: {
        ...
        informacoes: [
            ...
            "💳 *Transferência:* Itaú - Ag 1234, CC 56789-0",
...
client.on('message', async (msg) => {
    ...
    if (isAdmin(numero)) {
        console.log(`👤 Admin ${numeroLimpo} enviou mensagem - NÃO RESPONDER`);
        // IMPORTANTE: Não marca como humano ainda, apenas não responde
        return;
    }

    // ===== VERIFICAR SE O CHAT JÁ TEM HUMANO =====
    if (CHATS_COM_HUMANO.has(chatId)) {
...
// ========== DETECTAR QUANDO ADMIN RESPONDE E MARCAR CHAT COMO HUMANO ==========
client.on('message', async (msg) => {
